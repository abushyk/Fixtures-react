...
export default function App() {   
    ...
    const [globalData, setGlobalData] = useState({
        teams: [], // массив команд
        teamIndex: [], // массив связок ID команды - имя команды
        fixtures: [], // список результатов матчей
        tours: [] // список туров
    });

    // Финальная таблица с сортировкой и отбором по туру
    ...
    // Текущий тур
    const [showedTour, setShowedTour] = useState(1);

    // Управление модалкой информации
    const handleClose = () => setModalInfo({...modalInfo, show: false, matches: []});
    ...
    useEffect(() => {
        ...
        for (let team of teams) {
            globs.teamIndex[team.id] = team.name;
        }

        for (let fixture of fixtures) {
            globs.tours[fixture[0]] = fixture[0];
        }
        globs.tours = globs.tours.filter(item => item).sort((a,b) => a-b);

        let currentTour = globs.tours[globs.tours.length - 1];

        setShowedTour(currentTour);

        setGlobalData(globs); 
        
        //globalData = globs

        setResultTable(creatFixtureTable(globs.fixtures, globs.teamIndex));
        console.log('INIT');
    }, []);

    const buildResultTable = () => {
        let fixts = globalData.fixtures.filter((item) => item[0] <= showedTour)
        setResultTable(creatFixtureTable(fixts, globalData.teamIndex));
    }
    ...
    const showMatches = (teamID, matchType) => {
        ...
        let matches = globalData.fixtures.filter((fixture) => fixture[0] <= showedTour).filter((fixture) => {
            let res = false;
            switch (matchType) {
                case "wins": {
    ...
    const changeTourSelect = (event) => {
        ...
        setShowedTour(tour)
        let fixts = globalData.fixtures.filter((item) => item[0] <= tour)
        setResultTable(creatFixtureTable(fixts, globalData.teamIndex));
    }
    ...
    return (
        <>
            <Container>
                ...
                <Row>
                    <Col md={4}>
                        <TeamList items={globalData.teams} />
                    </Col>
                    <Col md={8}>
                        <TourSelector showedTour={showedTour} changeTourSelect={changeTourSelect} tours={globalData.tours} />
                        <ResultTable
                            fixturesData={resultTable}
